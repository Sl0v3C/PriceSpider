...
class AMZPrice(object):
    ...
    def get_item_list(self, item_list):
        ...
        status = tree.xpath('//*[@id = "search"]//a[@class="a-link-normal"]//@href')
        if status:
            for i in status:
                if re.search('^/gp/', i) or re.search('/dp/', i):
                    i = "https://www.amazon.cn" + i

                if re.search('/gp/help/|/gp/bestsellers/|/mobile-apps/', i):
                    continue
    ...
    def get_product_name(self):
        ...
        tree = html.fromstring(self.html)
        # status = tree.xpath(
        # '//div[@id="a-page"]/div[@id="dp"]/div[@id="dp-container"]/div[@id="centerCol"]//span[@id="productTitle"]/text()')
