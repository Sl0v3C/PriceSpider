...
import re, JDprice, threading, sys, AMZprice, TMprice, BBprice
import socket # socket.setdefaulttimeout(60) can set the global socket default timeout
...
class HTMLinfo(object):
    ...
    def replaceGoods(self):
        ...
        product = urllib.parse.quote(self.product)
        self.URL = re.sub(GOODS, product, new)
...
def getResponse(req):
    success = False
    count = 0
    while count < 60 and not success:
        try:
    ...
    if not success:
        print("Cannot urlopen: ", req.full_url)
        sys.exit()
    return f
...
def getHTML(url):
    ...
    f = getResponse(req)
    newCharSet = f.info().get_param('charset')
    if newCharSet:
        charset = newCharSet
    r = f.read().decode(charset, 'ignore')
